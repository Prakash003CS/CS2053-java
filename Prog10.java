...
public class Prog10 {
    ...
    public static void main(String[] args) {
        if (args.length < 2) {
            ...
            System.out.println("  As server: java prog10 server <port>");
            System.out.println("  As client: java prog10 client <host> <port>");
            return;
        }
        ...
        try {
            if (mode.equals("server")) {
                ...
                if (args.length < 3) {
                    System.out.println("Client usage: java prog10 client <host> <port>");
                    return;
                }
                String host = args[1];
            ...
            try {
                ...
                while ((line = socketIn.readLine()) != null) {
                    ...
                    if ("exit".equalsIgnoreCase(line.trim())) {
                        // remote asked to exit — break and let main close resources
                        break;
                    }
                }
            } catch (IOException e) {
            ...
            try {
                ...
                while ((inputLine = consoleIn.readLine()) != null) {
                    ...
                    if ("exit".equalsIgnoreCase(inputLine.trim())) {
                        // user wants to exit — break to allow cleanup
                        break;
                    }
                }
            } catch (IOException e) {
                // console closed or error
            }
        });
        writer.setDaemon(true);
        writer.start();

        // Wait for either thread to finish (exit typed or connection closed)
        try {
            writer.join();
